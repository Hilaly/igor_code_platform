...
Самостоятельно активировать его модель не может. Селекторы агента применяются уже после этого
фильтра и не возвращают скрытый скил в каталог даже при точном `include`. Поле подходит только для
ресурса, который другой код читает или использует напрямую вне model-visible каталога.
