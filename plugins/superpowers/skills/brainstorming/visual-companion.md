...
**Launching the server in Sovereign:**

Run the script through `bash`. The default detached mode normally survives across turns:

```bash
scripts/start-server.sh --project-dir /path/to/project --open
```

If the environment reaps detached processes, add `--foreground` and call `bash` with
`run_in_background: true`. Use `job-output` to read startup output or `job-kill` to stop that job.
...
1. **Check server is alive**, then **write HTML** to a new file in `screen_dir`:
   ...
   - Use `write` — **never use cat/heredoc** (dumps noise into terminal)
