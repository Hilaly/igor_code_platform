...
Use `subagent-list` and `subagent-output` to inspect progress. Coordinate only when necessary with
`subagent-message`; stop a mis-scoped agent with `subagent-stop`.
