...
- [x] **Step 1: Красный тест на место и кардинальность**
...
- [x] **Step 2: Добавить значение и место**
...
- [x] **Step 3: Зелёный + `make check`**, коммит `feat(protocol): add the tabs cardinality and core.panel.tabs`.
...
- [x] **Step 1: Красный — страж `everyKind` ломает сборку**
...
- [x] **Step 2: Красный тест на смешанный порядок**
...
- [x] **Step 4: `make check`**, коммит `feat(protocol): register command contributions`.
...
- [x] **Step 1: Красный тест** — `contribute.command` доносит объявление до хоста без изменения формы;
      тест на совпадение двух копий `placeCardinalities`.
- [x] **Step 2: Реализовать, `make check`**, коммит `feat(sdk): let plugins contribute commands`.
...
- [x] **Step 1: Красные тесты на отказы** — без `title`, без `export`, у плагина без `sovereign.browser`,
      с `placeId` не по шаблону. Каждый отказ несёт внятную причину.
- [x] **Step 2: Красные тесты на приём** — команда с несуществующим `placeId` регистрируется и ждёт;
...
- [x] **Step 3: Реализовать, `make check`**, коммит `feat(daemon): validate command contributions`.
...
- [x] **Step 2: Реализовать.** `content` — созданный, но не смонтированный `PlaceInstance`.
- [x] **Step 3: `make check`**, коммит `feat(browser-sdk): expose place tabs`.
...
- [x] **Step 2: Реализовать.** `invoke` не бросает никогда.
- [x] **Step 3: `make check`**, коммит `feat(browser-sdk): invoke plugin commands`.
...
- [x] **Step 1: Красные тесты** — кнопка нарисована по `title` до загрузки бандла; щелчок вызывает
      команду; после загрузки `available → false` выключает кнопку; кнопка команды и компонент стоят в
      одной полосе в общем порядке.
- [x] **Step 2: Реализовать, `make check`**, коммит `feat(browser-sdk): render command buttons in action places`.
...
- [x] **Step 2: Реализовать.** Контекст — `pageContext`, без `project`.
- [x] **Step 3: `make check`**, два коммита: `fix(web): stop announcing the tab strip as a tablist` и
...
- [x] **Step 1: Красные тесты** — у каждой команды есть строка в обоих каталогах; идентификаторы
...
- [x] **Step 2: Реализовать, `make check`**, коммит `feat(web): declare core commands`.
...
- [x] **Step 1: Красные тесты** — открытие кнопкой и аккордом Cmd/Ctrl+K (с `preventDefault`);
      ...
      выключена.
- [x] **Step 2: Реализовать, `make check`**, коммит `feat(web): add the command palette`.
...
- [x] **Step 1: Красные тесты** — команда показана видом и техническими данными; её размещение попадает
      в раздел мест исходом `added`; `plugins.kind.command` есть в обоих каталогах (существующий
      `i18n.test.ts` краснеет сам).
- [x] **Step 2: Реализовать, `make check`**, коммит `feat(web): show command contributions in the plugin view`.
...
- [x] **Step 1: Фикстура** — `placed` получает вкладку в `core.panel.tabs` и команду с
      `placeId: "core.view.header.actions"`, плюс экспорты `BoardTab` и `RunCommand` в `browser.tsx`.
- [x] **Step 2: Живая проверка** на собранном демоне из чистой временной директории данных по сценарию
...
- [x] **Step 3: Замер отменён.** Отдельной проверки рантайма палитра не требует: она читает только
      снимок и `load` не зовёт вовсе, поэтому «сколько бандлов грузит открытие палитры» — вопрос
...
- [x] **Step 4: Документы.** Отдельно: снять из `docs/roadmap.md` устаревшее «в 12c набор кита
...
- [x] **Step 5: `make check`, `make build`**, коммиты по темам.

---

### Task 13: Сверка ветки без переписывания локальной истории

- [x] **Step 1: Переписывание отменено как ненужное.** Двенадцать продуктовых коммитов уже были
      атомарны и каждый создавался после зелёного `make check`; backup-ref не создавался, потому что
      операции над историей не выполнялись.
- [x] **Step 2: Дерево и живая проверка сверены без rewrite.** `git diff --check`, `make check`,
      `make build` и сценарий из runbook прошли на существующей истории.
- [x] **Step 3: Публикация не выполнялась.** `push` и PR не сделаны; они остаются только по явной
      просьбе владельца продукта.
