...
### Повторное ревью: dot-сегменты провайдера

- [x] Воспроизвести через настоящий `history.pushState`, что `.` и `..` нормализуются браузером до
      другой страницы.
- [x] Добавить сегментный кодек, который отличает `.` и `..` от обычных идентификаторов `~.` и
      `~..`, и интеграционный тест полного круга.
- [x] Обновить устаревшее имя теста формы и документацию маршрута.
- [x] Повторить полную проверку и независимое ревью перед merge.
