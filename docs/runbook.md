...
- **демон** — `http://127.0.0.1:8787`, перезапускается сам при правке исходников (`node --watch`);
...
- Разбора аргументов запуска и директории данных: решено, что это два аргумента — путь и порт,
  каждый со значением по умолчанию ([ADR-0034](adr/0034-port-and-data-directory-come-from-launch-arguments.md)),
  но кода нет, и порт временно берётся из `SOVEREIGN_PORT`.
