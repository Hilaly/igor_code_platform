...
Внешний плагин выключен, пока его не включили руками:

```bash
echo '{ "plugins": { "data:hello": { "enabled": true } } }' > apps/daemon/.sovereign-dev/preferences.json
```

Демон подхватит правку без перезапуска, поднимет воркер и дальше будет перезагружать плагин на каждую
правку его исходников. Что происходит — видно в журнале демона: переходы жизненного цикла идут
записями `plugin lifecycle`, строки самого плагина — с источником `plugin:<id>`. Набор применённых
вкладов пишется на уровне `debug`:

```bash
echo '{ "logLevel": "debug" }' > apps/daemon/.sovereign-dev/config.json
```

Вью плагинов ещё нет, поэтому журнал — единственный способ увидеть состояние.
...
- Авторизации — раздел спецификации ещё не начат.
- Из плагинной системы нет типизированных видов вкладов, шины, HTTP-маршрутов плагина, его хранилища
  и браузерной части — перечень в [plugins.md](plugins.md).
