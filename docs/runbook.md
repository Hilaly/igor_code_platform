...
9. **Палитра работает на установке без единого плагина.** Выключите `placed`, `rival` и
   `browserless` целиком — палитра по-прежнему показывает двенадцать команд ядра. Ради этого команды
...
- Продакшн-сборки единым приложением: решено, что в артефакт вшиты статика фронтенда и исходники
  ...
  веб-интерфейс.
- У UI плагинов ещё нет страниц на `/p/<id>/` и фасада навигации — это срез 12c-2. Браузерные
  бандлы, публичный browser SDK, места ядра и плагинов, component-вклады, вкладки правой панели и
  команды уже работают ([ui-extension-model.md](ui-extension-model.md)).
