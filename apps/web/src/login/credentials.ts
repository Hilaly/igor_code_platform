/**
 * Что не так с введённым в форму входа. Отдельно от разметки, потому что DOM-стека в тестах нет
 * (docs/backlog.md), а порядок жалоб — это как раз то, что видно только проверкой: «пароли не
 * совпадают» не должно загораться на первом символе первого поля.
 *
