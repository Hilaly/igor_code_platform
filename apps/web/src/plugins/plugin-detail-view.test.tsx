...
const translator = createTranslator({
  ...
  onDiagnostic: (diagnostic) => { throw new Error(diagnostic); },
});

const snapshot: PluginsSnapshot = {
  revision: 1,
  plugins: [{ key: "data:example", id: "example", source: "data", directory: "/plugins/example", state: "running", attempt: 1, contributionProblems: ["bad contribution"] }],
  contributions: [{ kind: "event", ownership: "plugin", pluginKey: "data:example", pluginId: "example", source: "data", id: "example.event", declaredId: "event", title: "Example event", payloadSchema: { type: "object" } }],
  switchedOffContributions: [{ kind: "skill", ownership: "plugin", pluginKey: "data:example", pluginId: "example", source: "data", id: "example.skill", declaredId: "skill", title: "Example skill", name: "skill", location: "/plugins/example/skill", disableModelInvocation: false }],
  conflicts: [],
...
it("shows plugin facts, controls each contribution, and exposes technical data", () => {
  ...
  const onBack = vi.fn();
  render(<PluginDetailView state={{ snapshot, stale: false }} pluginKey="data:example" onBack={onBack} onSwitch={onSwitch} translator={translator} />);
  ...
  expect(onSwitch).toHaveBeenCalledWith("data:example", { enabled: true, disabledContributions: ["missing.id", "example.event"] });
...
it("shows a not-found state for an unknown plugin key", () => {
  render(<PluginDetailView state={{ snapshot, stale: false }} pluginKey="data:nope" onBack={vi.fn()} onSwitch={vi.fn()} translator={translator} />);
