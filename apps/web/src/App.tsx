 ...
 * (ADR-0017), и владелец у него один.
 */
...
export function App() {
  ...
  // дерева, поэтому переключение стоит одинаково на пустой странице и на полной (ADR-0028).
  useEffect(() => {
    applyAppearance({
      preferences,
      prefersDark,
  ...
  useEffect(() => {
    ...
    const unsubscribe = bus.subscribe((event) => {
      ...
      // Пропуск в потоке — повод перезапросить своё состояние, а не показывать неполное (ADR-0038).
