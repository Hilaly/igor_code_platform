/**
 * Обнаружение плагинов в корнях-источниках (ADR-0036). Источник — не просто место на диске: он же
 * ранг специфичности при споре вкладов (ADR-0030) и часть ключа плагина, потому что перекрывающая
 * копия — это другой плагин со своим включением и своим хранилищем (ADR-0067).
 *
...
/** Плагин объявлен, но взять его нельзя — и это не решение человека (ADR-0044). */
...
/**
 * Встроенный источник — каталог `plugins/` репозитория (ADR-0036). В продакшн-сборке он приедет
...
export function discoverPlugins(roots: PluginRoot[]): PluginDiscovery {
  ...
  for (const root of roots) {
    ...
    // чему — ранг у них общий (ADR-0030). Оба отказаны, обе папки названы.
