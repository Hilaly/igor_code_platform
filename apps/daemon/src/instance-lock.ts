/**
 * Единственность экземпляра (ADR-0008). Делимый ресурс — директория данных, а не порт, поэтому
...
export function acquireInstanceLock(options: AcquireInstanceLockOptions): InstanceLock {
  ...
  // одновременно стартующих демона по очереди сносили бы лок друг друга (ADR-0008).
  unlinkSync(path);
...
/**
 * Случай, которого ADR-0008 не описал: файл есть, но прочитать его нечем — демон умер между
