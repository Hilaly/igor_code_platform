/**
 * Журнал наружу не отдаётся: единственный его получатель — `stdout` демона (ADR-0074). Форма записи
 ...
 * Подставляет его ядро, а не плагин (ADR-0074).
 */
...
export type LogRecord = {
  ...
  source: LogSource;
  message: string;
  /** Произвольные дополнительные поля записи. Секреты в лог не попадают (ADR-0074). */
